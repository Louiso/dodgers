...
import {
  useCallback, useRef, useDebugValue,
} from 'react'
import { defaultConfig } from './utils/config'
import {
  SWRGlobalState, GlobalState,
...
export const useSWRHandler = <Data = any, Error = any>(
  ...
  const [EVENT_REVALIDATORS, STATE_UPDATERS, MUTATION, FETCH] = SWRGlobalState.get(cache) as GlobalState
  ...
  const patchFetchInfo = (info: { isValidating?: boolean; error?: any }) => cache.set(keyInfo, mergeObjects(cache.get(keyInfo), info))
  ...
  const revalidate = useCallback(
    async (revalidateOpts?: RevalidatorOptions): Promise<boolean> => {
      ...
      try {
        ...
        // Clear error.
        patchFetchInfo({ error: UNDEFINED })
...
export const SWRConfig = OBJECT.defineProperty(ConfigProvider, 'default', { value: defaultConfig }) as typeof ConfigProvider & {
